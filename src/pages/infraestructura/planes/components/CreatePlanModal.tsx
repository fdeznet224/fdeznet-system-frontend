...
export default function PlanModal({ isOpen, plan, onClose, onSuccess, routers }: Props) {
    ...
    useEffect(() => {
        ...
        if (isOpen) {
            mountTimer = window.setTimeout(() => setIsMounted(true), 10);
            if (plan) {
                setFormData({
                    nombre: plan.nombre,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const payload = {
            ...
            router_id: Number.parseInt(formData.router_id, 10),
            garantia_percent: Number.parseInt(formData.garantia_percent, 10),
            prioridad: Number.parseInt(formData.prioridad, 10),
            ...
            burst_time: Number.parseInt(formData.burst_time, 10)
        };
...
const InputGroup = ({ label, value, onChange }: InputGroupProps) => (
    <div>
...
const InputSimple = ({ label, val, set }: InputSimpleProps) => (
    <div>
