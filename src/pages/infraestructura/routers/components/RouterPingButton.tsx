...
import type { RouterPingResponse } from '../types';
...
export default function RouterPingButton({ routerId }: { routerId: number }) {
    ...
    const checkPing = async () => {
        setStatus('checking');
        try {
            const { data } = await client.get<RouterPingResponse>(`/network/routers/${routerId}/ping`);
    ...
    return (
        <button 
            onClick={(e) => { e.stopPropagation(); void checkPing(); }}
