...
interface Props {
    ...
    onClose: () => void;
    onSuccess: () => void;
    sugerencia?: { caja_nap_id?: number; puerto_nap?: number };
}
...
/** Modal legado sin ruta activa; se conserva junto al módulo de órdenes para una futura integración. */
export default function CreateOrdenModal({ isOpen, onClose, onSuccess, sugerencia }: Props) {
        ...
        if (isOpen) {
            const cargarDatos = async () => {
                try {
                    ...
                    const listaTecnicos = resUsers.data.filter((usuario) => usuario.rol === 'tecnico');
    ...
    }, [isOpen, sugerencia?.caja_nap_id, sugerencia?.puerto_nap]);
    ...
    const notificarTecnico = (tel: string, data: OrderFormData) => {
        const zonaNombre = zonas.find((item) => item.id === Number(data.zona_id))?.nombre;
