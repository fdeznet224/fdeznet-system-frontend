...
import {
    ...
    MapPinIcon, GlobeAltIcon, UserCircleIcon,
...
export default function MensajesCRM() {
    ...
    const fetchDirectorio = async () => {
        try {
            const resClientes = await client.get('/clientes/listado-completo-unificado');
    ...
    useEffect(() => {
        ...
        const fetchChat = async () => {
            try {
                ...
                const res = await client.get(`/whatsapp/chat/${activeClienteId}`);
    ...
    useEffect(() => {
        ...
        if (wsEvent.type === 'NEW_MESSAGE') {
            ...
            if (msg.cliente_id === activeClienteId) {
                setMensajes(prev => {
                    if (prev.some(m => m.id === msg.id)) return prev;
                    return [...prev, msg];
                });
                client.get(`/whatsapp/chat/${activeClienteId}`).catch(()=>{});
                clearUnread(activeClienteId);
            }
        } 
    ...
    const handleEnviarMensaje = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            const res = await client.get(`/whatsapp/chat/${activeClienteId}`);
            setMensajes(res.data);
        } catch (error) { toast.error("Error al enviar mensaje"); } 
        finally { setSending(false); }
    };

    const handleToggleSuspension = async (cliente: any) => {
        ...
        try {
            ...
            fetchDirectorio(); 
        } catch (error) { toast.dismiss(load); toast.error("Error"); }
    };

    // 🔥 EXTRAER ZONAS DISPONIBLES
    const zonasDisponibles = useMemo(() => {
        const zonas = clientes.map(c => c.zona).filter(Boolean); 
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0 border-b border-slate-800/50 pb-3 pt-1">
                                    ...
                                    <div className="flex justify-between items-center">
                                        <p className="text-sm text-[#8696a0] truncate">{c.telefono}</p>
                                        {unread > 0 && (
