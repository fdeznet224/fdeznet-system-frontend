...
export default function Plantillas() {
    ...
    // 🔥 SE AGREGÓ EL TÍTULO PARA EL ABONO PARCIAL
        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-10 p-4 md:p-6 max-w-7xl mx-auto transition-colors duration-300">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                <div className="flex items-center gap-4 w-full md:w-auto">
                    <button onClick={() => navigate('/admin/configuracion')} className="p-2 bg-slate-100 dark:bg-slate-800 rounded-full text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors shrink-0">
                    ...
                    <div className="min-w-0">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 text-xs md:text-sm mt-1 truncate">Configura los mensajes automáticos que enviará el Bot.</p>
                    </div>
                </div>
                <button 
                    onClick={handleCreate}
                    className="w-full md:w-auto flex items-center justify-center gap-2 px-5 py-3 md:py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-black transition shadow-md active:scale-95 uppercase tracking-widest text-xs shrink-0"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {loading ? (
                    ...
                    <div className="col-span-1 md:col-span-2 xl:col-span-3 text-center py-16 bg-white dark:bg-slate-800/30 rounded-2xl border border-slate-200 dark:border-slate-700/50 border-dashed transition-colors">
                        ...
                        <div key={p.id} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden group hover:border-emerald-500/50 transition-all duration-300 shadow-sm dark:shadow-lg flex flex-col hover:-translate-y-1">
                            <div className="p-5 flex-1 flex flex-col">
                                ...
                                <div className="bg-slate-50 dark:bg-[#0b141a] p-4 rounded-xl border border-slate-200 dark:border-slate-700/50 relative mt-2 flex-1 shadow-inner transition-colors">
                                    ...
                                    <p className="text-[13px] text-slate-800 dark:text-slate-200 leading-relaxed whitespace-pre-wrap font-medium transition-colors break-words">
            ...
            {/* Banner Informativo */}
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 flex flex-col md:flex-row items-center md:items-start gap-6 shadow-sm dark:shadow-xl transition-colors">
                <div className="p-3 bg-indigo-50 dark:bg-indigo-500/10 rounded-full border border-indigo-100 dark:border-indigo-500/20 shrink-0">
                    <LightBulbIcon className="w-8 h-8 text-indigo-600 dark:text-indigo-400" />
                </div>
                <div className="flex-1 w-full">
                    <h3 className="text-lg font-black text-slate-900 dark:text-white mb-2 transition-colors text-center md:text-left">Variables Maestras de FdezNet</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mb-4 max-w-3xl text-center md:text-left">
                        Usa estas etiquetas en tus mensajes. El sistema inyectará los datos reales de cada cliente, equipo y transacciones automáticamente.
                    </p>
                    
                    <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                        {/* 🔥 SE AGREGARON LAS VARIABLES {monto_pagado} Y {referencia} AQUÍ */}
                        {[
                            '{nombre}', '{cedula}', '{plan}', '{precio}', '{dia_corte}', 
                            '{direccion}', '{onu_serial}', '{nodo}', '{ip}', 
                            '{usuario_pppoe}', '{pass_pppoe}', '{empresa}', '{fecha_actual}',
                            '{monto_pagado}', '{referencia}' 
                        ].map(tag => (
                            <span key={tag} className="px-2.5 py-1.5 bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg text-[10px] sm:text-xs font-mono text-emerald-600 dark:text-emerald-400 font-black select-all transition-colors cursor-copy hover:bg-emerald-50 dark:hover:bg-emerald-900/20" title="Clic para copiar">
                                {tag}
                            </span>
                        ))}
                    </div>
