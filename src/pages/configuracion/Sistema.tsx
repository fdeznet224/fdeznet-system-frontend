...
export default function Sistema() {
    ...
    return (
        <div className="max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-28 p-4 transition-colors duration-300">
            ...
            <div className="flex items-center gap-4 mb-8 border-b border-slate-200 dark:border-slate-800 pb-6 transition-colors">
                ...
                <div>
                    <h1 className="text-2xl font-black text-slate-900 dark:text-white flex items-center gap-2 transition-colors">
                        <Cog6ToothIcon className="w-6 h-6 text-purple-600 dark:text-purple-500" />
                        Sistema & Cronjobs
                    </h1>
                    <p className="text-slate-500 dark:text-slate-400 text-sm">Configuración global del servidor.</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                    <div className="bg-white dark:bg-[#0f1219] rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-xl relative overflow-hidden transition-colors">
                        ...
                        <div className="space-y-5 relative z-10">
                            ...
                            <div className="p-4 bg-slate-50 dark:bg-[#151b2b] rounded-xl border border-slate-200 dark:border-slate-800 transition-colors">
                    ...
                    <div className="bg-white dark:bg-[#0f1219] rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-xl transition-colors">
                ...
                {/* COLUMNA DERECHA */}
                <div className="bg-white dark:bg-[#0f1219] rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-xl relative overflow-hidden flex flex-col h-full transition-colors">
                    <div className="absolute top-0 right-0 p-4 opacity-[0.05] pointer-events-none">
                    ...
                    <div className="flex justify-between items-center mb-6 relative z-10">
                        ...
                        <div className="flex items-center gap-2 bg-slate-100 dark:bg-[#151b2b] px-3 py-1.5 rounded-full border border-slate-200 dark:border-slate-800">
                            <span className={`text-[10px] font-black uppercase ${config.activar_notificaciones ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-500'}`}>
                    ...
                    <div className={`space-y-4 relative z-10 transition-all ${!config.activar_notificaciones ? 'opacity-50 pointer-events-none' : ''}`}>
                        <div className="p-4 bg-emerald-50 dark:bg-[#0a271d] border border-emerald-200 dark:border-emerald-900/50 rounded-xl mb-6 flex items-start gap-3 transition-colors">
                            <ShieldCheckIcon className="w-5 h-5 text-emerald-600 dark:text-emerald-500 shrink-0 mt-0.5"/>
                            <p className="text-xs text-slate-600 dark:text-emerald-200/80 leading-relaxed font-medium">El bot enviará recordatorios automáticamente.</p>
                        </div>
            ...
            <div className="fixed bottom-6 left-0 right-0 flex justify-center z-50 pointer-events-none p-4">
                <div className="pointer-events-auto bg-white/90 dark:bg-[#0f1219]/90 backdrop-blur-md p-2 pl-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xl flex gap-6 items-center transition-colors">
                    <span className="text-xs font-black text-slate-500 hidden sm:block uppercase">Cambios pendientes</span>
                    <button onClick={guardarCambios} disabled={saving} className="bg-purple-600 hover:bg-purple-500 text-white font-black py-3 px-8 rounded-xl shadow-md active:scale-95 transition flex items-center gap-2 text-sm uppercase tracking-widest">
                        {saving ? 'Guardando...' : <><CheckCircleIcon className="w-5 h-5"/> Guardar Configuración</>}
                    </button>
                </div>
            </div>
        </div>
...
const Toggle = ({ checked, onChange, color = 'emerald' }: any) => {
    ...
    return (
        <div onClick={() => onChange(!checked)} className={`w-11 h-6 flex items-center rounded-full p-1 cursor-pointer transition-colors ${checked ? colors[color] : 'bg-slate-300 dark:bg-slate-700'}`}>
...
const CardInputRight = ({ label, sub, value, onChange, textColor }: any) => (
    <div className="bg-slate-50 dark:bg-[#151b2b] p-4 rounded-xl border border-slate-200 dark:border-slate-800 flex items-center justify-between transition-colors">
        <div>
            <label className={`text-sm font-black block ${textColor || 'text-slate-900 dark:text-white'}`}>{label}</label>
            <p className="text-[10px] text-slate-500 mt-1 uppercase tracking-widest font-black">{sub}</p>
        </div>
        <div className="relative flex items-center">
            <input type="number" className="w-16 bg-white dark:bg-[#0b0e14] border border-slate-200 dark:border-slate-700 rounded-lg py-2 px-3 text-slate-900 dark:text-white font-mono text-center text-lg font-black outline-none focus:border-purple-500 transition-colors" value={value} onChange={e => onChange(e.target.value)} />
        </div>
    </div>
);