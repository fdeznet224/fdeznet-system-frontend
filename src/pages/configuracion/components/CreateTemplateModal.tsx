...
interface TemplateInitialData extends Partial<FormState> {
    id?: number;
}
...
export default function CreateTemplateModal({ isOpen, onClose, onSuccess, initialData }: Props) {
    ...
    useEffect(() => {
        if (initialData) {
            setFormData({
                nombre: initialData.nombre ?? DEFAULT_FORM.nombre,
    ...
    const calcFechaGeneracion = () => {
        const dia = val(formData.dia_pago) - val(formData.dias_antes_emision);
                    ...
                    <div className="flex min-h-full items-center justify-center p-4">
                        <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                            <Dialog.Panel className="bg-white dark:bg-[#0f1219] w-full max-w-5xl rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl flex flex-col md:flex-row overflow-hidden transition-colors">
                                ...
                                <button onClick={onClose} aria-label="Cerrar ciclo" className="absolute top-4 right-4 text-slate-400 hover:text-slate-900 dark:hover:text-white z-10 transition-colors">
                                ...
                                <div className="flex-1 p-8 md:p-10">
                                    <div className="mb-8">
                                        <div className="flex items-center gap-3 mb-2">
                                            ...
                                            <Dialog.Title className="text-2xl font-black text-slate-900 dark:text-white transition-colors">
                                                {initialData?.id ? 'Editar Ciclo' : 'Nuevo Ciclo'}
                                            </Dialog.Title>
                                        </div>
                                    </div>
...
const InputNumber = ({ label, sub, value, onChange, color }: InputNumberProps) => (
