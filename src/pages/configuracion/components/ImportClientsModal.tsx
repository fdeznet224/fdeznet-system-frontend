...
/**
 * Flujo legado sin consumidor activo. La importación vigente vive en
 * pages/configuracion/Importar.tsx y admite CSV/Excel desde Configuración.
 */
export default function ImportClientsModal({ isOpen, onClose, routerId }: Props) {
    ...
    const [olts, setOlts] = useState<CatalogOption[]>([]);
            ...
            const loadOptions = async () => {
                ...
                try {
                    const [resRedes, resZonas, resPlantillas, resPlanes, resOlts] = await Promise.all([
                        client.get<CatalogOption[]>(`/network/redes/router/${routerId}`),
                        client.get<CatalogOption[]>('/config/zonas'),
                        client.get<CatalogOption[]>('/plantillas-facturacion/'),
                        client.get<CatalogOption[]>('/planes/'),
                        client.get<CatalogOption[]>('/olts/')
                    ]);
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            toast.error(getErrorMessage(error, "Error al procesar archivo"));
        } finally {
            setUploading(false);
        }
    };
