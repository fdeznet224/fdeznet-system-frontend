/** Chat compartido por clientes, órdenes y técnicos. */
...
export default function ChatModal({ isOpen, onClose, cliente, onMessagesRead }: ChatModalProps) {
    ...
    useEffect(() => {
        ...
        if (
            ...
            && typeof wsEvent.data.mensaje === 'string'
        ) {
            const incomingMessage: ChatMessage = {
                id: wsEvent.data.id,
                cliente_id: wsEvent.data.cliente_id,
                direccion: wsEvent.data.direccion || 'entrada',
                mensaje: wsEvent.data.mensaje,
                fecha: wsEvent.data.fecha,
                ack: wsEvent.data.ack,
            };
            setChatMessages(prev => [...prev, incomingMessage]);
            void client.get(`/whatsapp/chat/${clientId}`).catch(() => undefined);
            clearUnread(clientId);
            onMessagesRead?.();
        }
    ...
    return createPortal(
        <div role="dialog" aria-label={`Chat con ${cliente.nombre}`} aria-modal="true" className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/60 dark:bg-black/80 backdrop-blur-sm sm:p-4 transition-colors">
            {/* Modal Container */}
            <div className="w-full h-[85vh] sm:h-[650px] sm:max-w-md bg-white dark:bg-[#0b141a] flex flex-col rounded-t-[2.5rem] sm:rounded-3xl shadow-2xl border-t sm:border border-slate-200 dark:border-slate-800 transition-colors">
                ...
                <div className="bg-slate-50 dark:bg-[#202c33] px-4 py-3 flex items-center justify-between border-b border-slate-200 dark:border-slate-800 transition-colors shrink-0">
                    ...
                    <button aria-label="Cerrar chat" onClick={onClose} className="p-2 shrink-0 hover:bg-slate-200 dark:hover:bg-[#2a3942] rounded-full text-slate-500 dark:text-[#aebac1] transition-colors">
